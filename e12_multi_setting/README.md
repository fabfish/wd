...
## 红列矩形多种子验证（ms5–ms7b，2026-09-18）

热力图显示规则：任何有多种子数据（≥2 seeds）的格子显示 3-seed 均值并加粗，
单种子格子显示 seed-42 值。每张热力图红色列（指数映射 s≥0.75）矩形内的所有
格子已补满 3 种子（42/123/2024）。各 setting 红列矩形内每形状的最高 3-seed
均值：

| setting | phase | fixed | iso_up | linear_up | linear_down | 红列结论 |
|---|---|---|---:|---:|---:|---|
| resnet18/cifar100 | SGD | 78.03 | **78.54** | 78.18 | 77.71 | iso 稳定最高（+0.51） |
| resnet18/cifar100 | SGDM | 77.34 | **78.30** | 78.21 | 76.07 | iso 稳定最高（+0.96） |
| resnet34/cifar100 | SGD | 78.67 | 79.14 | **79.22** | 78.94 | lin_up 最高、iso 并列；fixed 落后 ~0.5 |
| resnet34/cifar100 | SGDM | 78.32 | **78.38** | 78.34 | 77.34 | iso 微优（对 fixed +0.06，噪声内） |
| resnet50/cifar100 | SGD | 79.59 | 79.67 | **79.87** | 79.40 | lin_up 稳定最高（+0.28） |
| resnet50/cifar100 | SGDM | 77.36 | **78.59** | 78.45 | 76.98 | iso 稳定最高（+1.23） |
| vgg13/cifar100 | SGD | 76.13 | **77.04** | 76.61 | 75.16 | iso 稳定最高（+0.91） |
| vgg13/cifar100 | SGDM | 74.58 | **75.17** | 74.66 | 73.61 | iso 稳定最高（+0.59） |
| vgg16/cifar100 | SGD | 75.23 | **76.61** | 76.37 | 74.90 | iso 稳定最高（+1.38） |
| vgg16/cifar100 | SGDM | 73.09 | **73.82** | 73.49 | 72.62 | iso 稳定最高（+0.73） |

要点：

- **10/10 setting 的红列最优均为动态 wd（9×iso、1×lin_up），fixed 无一居首**。
- 最接近的是 R34/SGDM：iso 78.38 vs fixed 78.32（+0.06，种子噪声内），但 iso
  的 0.90C 档区间（78.12~78.79）整体压住 fixed 的 1.00C 档（77.61~78.41）。
- seed-42 的 linear_up 峰值（79.54@0.90C 等）多种子后回落 0.2~0.5 分；iso 的
  回落与方差最小，是红区最稳的形状。
- 暖色区（橙黄列，s≥0.5）的多种子补齐在 ms8 队列进行中（100 runs）；完成后
  本小节将扩展为完整暖色区验证，并重生成全部热力图。
