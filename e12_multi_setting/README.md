...
## 暖色区多种子验证（ms5–ms8，2026-09-18）

热力图显示规则：任何有多种子数据（≥2 seeds）的格子显示 3-seed 均值并加粗，
单种子格子显示 seed-42 值。每张热力图的暖色区（列内 max s≥0.5，即所有离开
蓝色的红/橙/黄列）矩形内全部格子已补满 3 种子（42/123/2024，其中部分多
seed 来自 e12_fill 阶段的 123/2024 行）。各 setting 暖色区内每形状的最高
3-seed 均值：

| setting | phase | fixed | iso_up | linear_up | linear_down | 结论 |
|---|---|---|---:|---:|---:|---|
| resnet18/cifar100 | SGD | 78.03 | **78.54** | 78.18 | 77.71 | iso 最高（+0.51） |
| resnet18/cifar100 | SGDM | 77.34 | **78.30** | 78.21 | 76.48 | iso 最高（+0.96） |
| resnet34/cifar100 | SGD | 78.72 | 79.14 | **79.22** | 78.73 | lin_up 最高（+0.50）；iso 并列 |
| resnet34/cifar100 | SGDM | 78.32 | **78.38** | 78.34 | 77.34 | iso 微优（+0.06，噪声内） |
| resnet50/cifar100 | SGD | 79.59 | 79.67 | **79.87** | 79.40 | lin_up 最高（+0.28） |
| resnet50/cifar100 | SGDM | 77.66 | **78.59** | 78.45 | 76.98 | iso 最高（+0.93） |
| vgg13/cifar100 | SGD | 76.13 | **77.04** | 76.61 | 75.37 | iso 最高（+0.91） |
| vgg13/cifar100 | SGDM | 74.58 | **75.17** | 74.86 | 73.84 | iso 最高（+0.58） |
| vgg16/cifar100 | SGD | 75.23 | **76.61** | 76.17 | 74.76 | iso 最高（+1.38） |
| vgg16/cifar100 | SGDM | 73.09 | **73.82** | 73.49 | 72.62 | iso 最高（+0.73） |

要点：

- **10/10 setting 的暖色区最优均为动态 wd（9×iso、1×lin_up），fixed 无一
  居首**；三个 raise 形状（iso/lin_up/lin_down 高 C 区）普遍压住 fixed
  0.06~1.38 分。
- 唯一接近的是 R34/SGDM（iso 78.38 vs fixed 78.32，+0.06，种子噪声内），
  但 iso 的 0.90C 档区间（78.12~78.79）整体在 fixed 1.00C 档（77.61~78.41）
  之上。
- seed-42 的 linear_up 峰值多种子后回落 0.2~0.5 分；iso 的回落与方差最小，
  是暖色区最稳的形状。
- 暖色区之外的蓝色区域（低 C 档、collapse 尾）不做多种子：那里的格子在
  种子间摆动不改变任何结论，且全部形状均远离最优。
