...
> **2026-08-03 更正**：reviewer xkCF 指出 §2 的 joint multiplier 使
> `η_tλ_t = η₀λ₀m(t)²`，**并不保持**本文主张的耦合，因此不能用它来评价本文规则。
> 补做的 E9（保持 `η_tλ_t` 恒定的 iso-product 调度 + 按累积收缩匹配）见
> [`e9_iso_matched.md`](e9_iso_matched.md)：iso-product 达到 **78.22**，
> 高于常数 λ 的 per-cell oracle（77.28）与 joint cosine（76.42）。
> **主对照应改用 iso-product 臂，不再以 joint 为主。**

1. **缺 LR 退火时**，run内衰减 λ（或 η+λ 同乘）帮助很大——SGDM 固定 LR
   下 drop-step **+6.4pp**；joint cosine **+9.8pp**。
2. **一旦采用主设定（cosine LR + 常数 λ）**，默认 / `C/∑η` 已达
   **76.7%**，oracle **77.3%**，与 joint cosine **76.4%** 持平或更好。
3. **但保持耦合的调度更好**：`λ_t = λ₀η₀/η_t` 达**78.0–78.2%**（E9）。所以
   「schedule 与我们的规则互补」这个说法要精确化为：规则决定**收缩预算**该多大
   （`Σ_tη_tλ_t ≈ C`），调度决定这份预算**怎么在时间上花**；同预算下形状仍有
   1.7–3.5pp 差别，而最好的形状正是让 `η_tλ_t` 保持恒定的那个。
