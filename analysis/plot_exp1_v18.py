"""
Plot v18 for exp1 LR ordering.
Based on plot_exp1_v17.py.
Changes:
- Change SGDM+WD color to Yellow (#FBC02D).
"""
import pandas as pd
import numpy as np
...
def main():
    ...
    # COLOR SCHEME UPDATE
    # SGD: Dark Blue (#0D47A1) [v17]
    # SGDM (no WD): Orange (#FF9800)
    # SGD+WD: Mid Blue (#1E88E5)
    # SGDM+WD: Yellow/Gold (#FBC02D) [New]
    
    colors = ['#0D47A1', '#FF9800', '#1E88E5', '#FBC02D']
    ...
    for name, df, color, marker in final_curves:
        if df is not None and len(df) > 0:
            ...
            if len(df) > 0:
                ...
                if name == 'SGD':
                    # v17: kept side shift, reverted vertical
                    text_x += 0.05
    ...
    plt.tight_layout()
    output_path = output_dir / 'exp1_lr_ordering_v18.png'
