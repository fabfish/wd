...
For the LLM setting (Exp.~4), we perform a controlled sweep over learning rate $\eta$ and weight decay $\lambda$ for LoRA fine-tuning, while holding the remaining factors fixed (LoRA hyperparameters and schedule). We report SVAMP performance for stable runs to identify the high-accuracy region over $(\eta,\lambda)$ for a given batch size. We flag one caveat explicitly: in LoRA fine-tuning weight decay is not a load-bearing hyperparameter and is commonly set to zero, so agreement with the predicted coupling in this setting is weaker evidence than the vision experiments. We therefore present Exp.~4 as a suggestive observation rather than as validation of the coupling, and we do not draw quantitative conclusions from it.
