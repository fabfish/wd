"""Exp2 with widened stability margin -- gentle spoons.

Key changes vs run_exp2_smooth:
    momentum:   0.9  ->  0.5    (doubles the stable LR window)
    grad_clip: 10.0  ->  2.0    (bounds the per-step update tightly)

Combined, these keep training stable far past the original SGDM stability
threshold, so each spoon ascends gradually instead of falling off a cliff
to random-predictor loss.

Same eta x lambda grid as run_exp2_smooth (base 6x6 + extension for small wd).

Run after the smooth runs:
    python -m mlp_wd.scripts.run_exp2_gentle --gpus 1,2,3 --workers_per_gpu 8 \
        --loader_workers 0 --epochs 30 --grad_clip 2.0 --momentum 0.5
"""
...
def main():
    ...
    ap.add_argument("--momentum", type=float, default=0.5,
                    help="Smaller momentum widens the stability boundary")
    ap.add_argument("--epochs", type=int, default=30)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--grad_clip", type=float, default=2.0,
                    help="Tight grad-norm clip; pass <=0 to disable")
    ...
    ap.add_argument("--output", type=str,
                    default="mlp_wd/outputs/results/exp2_gentle.csv")
    ap.add_argument("--history_dir", type=str,
                    default="mlp_wd/outputs/history/exp2_gentle")
    ...
    print(f"[exp2-gentle] {len(rows)} runs | momentum={args.momentum} grad_clip={grad_clip}")
    run_grid(
        rows,
