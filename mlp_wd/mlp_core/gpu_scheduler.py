...
class GPUScheduler:
    ...
    def __init__(self, gpu_ids=None, verbose=False, workers_per_gpu=1,
        ...
        Args:
            ...
            threads_per_worker: Cap for OMP/MKL/OpenBLAS threads per worker.
                                If None, picked automatically as
                                max(2, nproc // total_workers // 2)
                                so the pool fits on the box without
                                oversubscribing.
        """
        if gpu_ids is None:
        ...
        total_workers = max(1, len(self.gpu_ids) * self.workers_per_gpu)
        if threads_per_worker is None:
            try:
                ncores = len(os.sched_getaffinity(0))
            except AttributeError:
                ncores = os.cpu_count() or 16
            # Pin total CPU thread budget to ~ncores so we don't oversubscribe.
            # Cap per-worker at 8 threads -- past that there's no measurable speedup
            # for this tiny MLP and only more contention.
            threads_per_worker = max(2, min(8, ncores // total_workers))
        ...
        if not self.gpu_ids:
            ...
            print(f"  Workers per GPU: {self.workers_per_gpu} "
                  f"(Total workers: {total_workers}, "
                  f"threads_per_worker: {self.threads_per_worker})")

    def _worker(self, gpu_id, worker_idx, task_queue, result_queue, worker_func, threads_per_worker=None):
        """
        ...
        Args:
            ...
            threads_per_worker: Cap for OMP/MKL/OpenBLAS thread pools.
                                None -> leave whatever the caller set.
        """
        # Set this process to use only the assigned GPU
        os.environ['CUDA_VISIBLE_DEVICES'] = str(gpu_id)

        # Cap CPU thread pools so concurrent workers don't oversubscribe and
        # collapse on context switches. The parent computes a sensible value
        # based on total worker count and available cores.
        if threads_per_worker is not None and threads_per_worker > 0:
    ...
    def run_tasks(self, tasks, worker_func, on_complete=None):
        ...
        for gpu_id in self.gpu_ids:
            for worker_idx in range(self.workers_per_gpu):
                p = mp.Process(
                    target=self._worker,
                    args=(gpu_id, worker_idx, task_queue, result_queue,
                          worker_func, self.threads_per_worker),
                )
                p.start()
                processes.append(p)
