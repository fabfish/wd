"""
Plot v5 for exp1 LR ordering using all available data + SGDM (no WD).
This version selects the BEST Momentum for SGDM (no WD) from the search results.
...
def main():
    ...
    if sgdm_search_file.exists():
        ...
        if len(df_search) > 0:
            ...
            # Find best momentum
            # Heuristic: 
            # 1. Curve must have optimal LR < 0.3 (SGD) AND > 0.1 (SGD+WD)
            # 2. Maximize accuracy
            ...
            for mom in momentums:
                ...
                if len(curve) < 3: continue # valid curve needs points
                
                best_pt = curve.loc[curve['best_test_acc'].idxmax()]
                peak_lr = best_pt['lr']
                acc = best_pt['best_test_acc']
                
                # Score: Accuracy penalized if LR is out of bounds?
                # Or just stiff filter:
                score = acc
                if peak_lr >= 0.3 or peak_lr <= 0.1:
                    score -= 5.0 # Penalize bad placement
            ...
            if best_mom != -1:
                ...
                print(f"Selected Best SGDM: m={best_mom}")
                 ...
                 # Fallback: just best acc
                 best_idx = df_search['best_test_acc'].idxmax()
                 best_mom = df_search.loc[best_idx, 'momentum']
                 sgdm_best_curve = df_search[df_search['momentum'] == best_mom].sort_values('lr')
                 sgdm_best_name = f'SGDM (no WD, m={best_mom})'
                 print(f"Fallback Selected SGDM: m={best_mom}")
                 
    else:
        print("Warning: sgdm_no_wd_search.csv not found")
    ...
    plt.tight_layout()
    output_path = output_dir / 'exp1_lr_ordering_v5.png'
