"""
Plot v16 for exp1 LR ordering.
Based on plot_exp1_v15.py.
Changes:
- Change x-axis tick label '0.4' to '0.50'.
- Shift SGD annotation DOWN (instead of up) to avoid overlap with upper curves.
  User said "down half a grid". Assuming grid is ~1-2 units, dropping by ~1.0 and setting va='top' should work.
"""
import pandas as pd
import numpy as np
...
def main():
    ...
    if sgdm_nowd_file.exists():
        ...
        # Explicitly remove LR=0.4 if present (v13 req)
    ...
    for name, df, color, marker in final_curves:
        if df is not None and len(df) > 0:
            ...
            if len(df) > 0:
                ...
                if name == 'SGD':
                    # User: "sgd 的标要往下一点... 往下半格"
                    # Shift text right (existing v13 logic)
                    text_x += 0.05
                    
                    # Shift text DOWN
                    text_y = best['best_test_acc'] - 0.7 # Approx half grid down (assuming grid gap ~1.0-2.0?)
                    va = 'top'
                else:
                    text_y = best['best_test_acc'] + 0.4
                    va = 'bottom'
    ...
    visual_labels = ['0.0', '0.05', '0.1', '0.15', '0.2', '0.3', '0.50'] # Changed 0.4 -> 0.50
    
    ax.set_xticks(visual_ticks)
    ax.set_xticklabels(visual_labels)
    ...
    plt.tight_layout()
    output_path = output_dir / 'exp1_lr_ordering_v16.png'
