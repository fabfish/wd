"""
Plot v11 for exp1 LR ordering.
Based on plot_exp1_v10.py.
Changes:
- Remove title.
- Remove "(no wd, m=0.1)" from SGDM legend label.
- Shift SGD annotation text to the right by half a grid (0.025).
"""
import pandas as pd
import numpy as np
...
def main():
    ...
    sgdwd_002_supp = pd.concat([df_sgdwd_supp, df_sgdwd_old]) # Just reading again to be safe involves merging logic if needed but user logic was specific in v10.
    # Actually let's just copy v10 logic exactly for data loading.
    
    # V10 logic for sgdwd_002:
    sgdwd_002_base = df_sgdwd_old[(df_sgdwd_old['method'] == 'SGD+WD') & (df_sgdwd_old['wd'] == 0.002)]
    ...
    sgdm_nowd_name = 'SGDM' # Default name if file not found or logic changes
    ...
    if sgdm_nowd_file.exists():
        ...
        sgdm_nowd_name = 'SGDM' # User requested removal of "(no wd, m=0.1)"
    ...
    for name, df, color, marker in final_curves:
        if df is not None and len(df) > 0:
            ...
            if len(df) > 0:
                ...
                # User request: "sgd 的 lr 和 acc 重叠了，往右移动半格"
                if name == 'SGD':
                    # Half a grid cell. Ticks are 0.0, 0.05, etc. Gap is 0.05. Half is 0.025.
                    text_x += 0.025
    ...
    plt.tight_layout()
    output_path = output_dir / 'exp1_lr_ordering_v11.png'
