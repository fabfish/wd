"""
Plot v15 for exp1 LR ordering.
Based on plot_exp1_v13.py.
Changes:
- Include LR=0.5 for all curves.
- Map LR=0.5 to visual position x=0.4.
- Keep SGDM (no WD) LR=0.4 excluded.
"""
import pandas as pd
import numpy as np
...
def main():
    ...
    # We want to keep 0.5.
    # The intersection logic in v13 was:
    common_lrs = lrs_sgd.intersection(lrs_sgdwd).intersection(lrs_sgdm)
    # If 0.5 is in all of them, it survives.
    # If it's missing from some, it gets dropped.
    # Given the user specifically asked for points at 0.5, we should ensure they are included if available in THAT curve, even if not common?
    # Usually "ordering" plots imply comparing at same LRs.
    # But for 0.5, maybe we just include it where possible.
    # However, strict intersection is safer for fairness. Let's assume 0.5 is common (it was in log).
    ...
    # Note: SGDM (no WD) is NOT filtered by common_lrs in v13, so we leave it as is.
    if sgdm_nowd_curve is not None:
    ...
    def map_lr(lr):
        # User request: "note 0.5 at 0.4's actual position"
        if np.isclose(lr, 0.5):
            return 0.4
        return lr
    ...
    for name, df, color, marker in final_curves:
        if df is not None and len(df) > 0:
            # Drop duplications if any
            df = df.drop_duplicates('lr')

            # CHANGED: Do NOT filter lr < 0.5.
            # Instead, allow 0.5, but map it.
            
            # Map LRs to X vals
            df['x_vals'] = df['lr'].apply(map_lr)
            
            # Sort by intended x position
            df = df.sort_values('x_vals')
            
            x_vals = df['x_vals']
            y_vals = df['best_test_acc']
            
            # Line plot
            ax.plot(x_vals, y_vals, marker=marker, label=name, color=color, lw=4.5, ms=14, alpha=0.9)
            ...
            if len(df) > 0:
                ...
                # User request from v13: "move right by another half grid" (total 0.05)
                if name == 'SGD':
                    text_x += 0.05
    ...
    # User request from v12/13: "Optimal LR and Accuracy"
    ...
    plt.tight_layout()
    output_path = output_dir / 'exp1_lr_ordering_v15.png'
