"""
Plot v17 for exp1 LR ordering.
Based on plot_exp1_v16.py.
Changes:
- Revert SGD annotation position (move back up).
- Swap colors of SGD and SGDM+WD.
- Keep x-axis label '0.50'.
"""
import pandas as pd
import numpy as np
...
def main():
    ...
    # Changed Color Scheme (Swapped 0 and 3)
    # Old: ['#90CAF9', '#FF9800', '#1E88E5', '#0D47A1']
    # New: ['#0D47A1', '#FF9800', '#1E88E5', '#90CAF9']
    
    colors = ['#0D47A1', '#FF9800', '#1E88E5', '#90CAF9']
    ...
    for name, df, color, marker in final_curves:
        if df is not None and len(df) > 0:
            ...
            if len(df) > 0:
                ...
                text_y = best['best_test_acc'] + 0.4 # Reverted to UP (default)
                va = 'bottom' # Reverted to bottom
                
                if name == 'SGD':
                    # User said: "move down too much, restore it" -> Back to ABOVE.
                    # Keep the X-shift (from v15)
                    text_x += 0.05
    ...
    visual_labels = ['0.0', '0.05', '0.1', '0.15', '0.2', '0.3', '0.50']
    ...
    plt.tight_layout()
    output_path = output_dir / 'exp1_lr_ordering_v17.png'
